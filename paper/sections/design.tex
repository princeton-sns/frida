...
\todo{need to prove that you are able to detect reordering within at most two round trips between the two devices. Prove that this is essentially sequential consistency }

\todo{
    \begin{itemize}
        \item data structure ->
        \item how do we detect reordering
        \item how and when to trim
        \item when to snapshot / agree on a snapshot
        \item how do we show consistency
        \item overheads of trimming
        \item upper bounds on storage
    \end{itemize}
}


\parlabel{Data Structure}
...
\parlabel{Adding Event Recieved}
...
\parlabel{Adding event initiated on own device}
